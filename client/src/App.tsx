...
const fetchMyProfile = async () => { try { const r = await fetch(`${API}/me`, { headers: { Authorization: `Bearer ${token}` } }); if (r.ok) { const d = await r.json(); setMyProfile(d); setEditDisplayName(d.displayName); setEditAvatarUrl(d.avatarUrl); } } catch { console.error("Profile fetch failed"); } };
const fetchRooms = async () => { try { const r = await fetch(`${API}/rooms`); if (r.ok) setAvailableRooms(await r.json()); } catch { console.error("Rooms fetch failed"); } };
...
const handleLogin = async (e: React.FormEvent) => { e.preventDefault(); try { const r = await fetch(`${API}/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: usernameInput, password: passwordInput }) }); const d = await r.json(); if (r.ok) { setToken(d.token); localStorage.setItem("token", d.token); setUsernameInput(""); setPasswordInput(""); setAuthError(""); } else setAuthError(d.error); } catch { setAuthError("Error"); } };
const handleRegister = async (e: React.FormEvent) => { e.preventDefault(); try { const r = await fetch(`${API}/register`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: usernameInput, password: passwordInput }) }); const d = await r.json(); if (r.ok) setAuthError("Registered."); else setAuthError(d.error); } catch { setAuthError("Error"); } };
const handleLogout = () => { setToken(""); localStorage.removeItem("token"); setMyProfile(null); localStorage.removeItem("lastRoomId"); };
const handleSaveProfile = async () => { try { await fetch(`${API}/profile`, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ displayName: editDisplayName, avatarUrl: editAvatarUrl }) }); setIsEditModalOpen(false); fetchMyProfile(); } catch { console.error("Save failed"); } };
...
const handleViewOtherProfile = async (u: string) => { try { const r = await fetch(`${API}/profile/${u}`); if (r.ok) { setViewingProfile(await r.json()); setViewProfileModalOpen(true); } else alert("Failed"); } catch { alert("Error"); } };
...
const handleSurrender = () => { socket?.emit("surrender"); };
...
const debtAmount = gameState?.activeAction?.type === "PAY" ? (gameState.activeAction.data?.amount || 0) : 0;
const myPlayerState = myProfile ? gameState?.players.find(p => p.userId === myProfile.id) : null;
const totalCapital = myPlayerState ? calculateTotalCapital(board, myPlayerState) : 0;
const cannotAffordEvenWithAssets = debtAmount > 0 && totalCapital < debtAmount;
...
const isOwner = selectedCell.ownerId === myProfile?.id;
const groupCells = board.filter(c => c.group === selectedCell.group && c.type === 'PROPERTY');
const ownedCount = groupCells.filter(c => c.ownerId === selectedCell.ownerId && !c.isMortgaged).length;
const isMonopoly = ownedCount === groupCells.length && selectedCell.ownerId !== null;
const isPartial = !isMonopoly && ownedCount > 0 && selectedCell.ownerId !== null;
let activeBase = false, activePartial = false, activeMonopoly = false;
if (isOwner && selectedCell.houses === 0) {
  activeBase = true;
  if (isPartial) activePartial = true;
  if (isMonopoly) { activeBase = false; activeMonopoly = true; }
}
return <><div style={(isOwner && activeBase && !activePartial && !activeMonopoly) ? styles.rentRowActive : styles.rentRow}><span style={{color: (isOwner && activeBase && !activePartial && !activeMonopoly) ? '#fff' : '#888'}}>Base:</span><span style={{color: (isOwner && activeBase && !activePartial && !activeMonopoly) ? '#fff' : '#eee'}}>${selectedCell.baseRent || 0}</span></div>{selectedCell.partialMonopolyRent && <div style={(isOwner && activePartial) ? styles.rentRowActive : styles.rentRow}><span style={{color: (isOwner && activePartial) ? '#fff' : '#888'}}>Partial:</span><span style={{color: (isOwner && activePartial) ? '#fff' : '#eee'}}>${selectedCell.partialMonopolyRent}</span></div>}{selectedCell.monopolyRent && <div style={(isOwner && activeMonopoly) ? styles.rentRowActive : styles.rentRow}><span style={{color: (isOwner && activeMonopoly) ? '#fff' : '#888'}}>Monopoly:</span><span style={{color: (isOwner && activeMonopoly) ? '#fff' : '#eee'}}>${selectedCell.monopolyRent}</span></div>}</>;
})()}</div>
<div style={styles.rentBlock}>{[1,2,3,4].map(n => {
const currentHouses = selectedCell.houses || 0;
const isActive = n === currentHouses && selectedCell.ownerId === myProfile?.id;
...
const isActive = n === ownedStations && selectedCell.ownerId === myProfile?.id;
...
const isActive = n === ownedUtilities && selectedCell.ownerId === myProfile?.id;
...
fontWeight: !canAffordPay && cannotAffordEvenWithAssets ? 700 : 400,
opacity: canAffordPay ? 0.4 : 1,
cursor: canAffordPay ? 'not-allowed' : 'pointer'
}}
>
Сдаться
</button>
