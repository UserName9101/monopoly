...
if (index >= 1 && index <= 12) return { position: "absolute", top: OFFSET + (index - 1) * STEP, left: 0, width: CELL, height: CORNER };
if (index >= 14 && index <= 25) return { position: "absolute", top: EDGE, left: OFFSET + (index - 14) * STEP, width: CORNER, height: CELL };
if (index >= 27 && index <= 38) return { position: "absolute", top: OFFSET + (38 - index) * STEP, left: EDGE, width: CELL, height: CORNER };
return { position: "absolute", top: OFFSET + (51 - index) * STEP, right: 0, width: CORNER, height: CELL };
}

function getStripStyle(index: number, color: string): React.CSSProperties {
if (index >= 1 && index <= 12) return { position: "absolute", right: 0, top: 0, bottom: 0, width: STRIP_SIZE, background: color };
if (index >= 14 && index <= 25) return { position: "absolute", bottom: 0, left: 0, right: 0, height: STRIP_SIZE, background: color };
if (index >= 27 && index <= 38) return { position: "absolute", left: 0, top: 0, bottom: 0, width: STRIP_SIZE, background: color };
return { position: "absolute", top: 0, left: 0, right: 0, height: STRIP_SIZE, background: color };
}
...
const contentPadding = hasStrip ? (i >= 1 && i <= 12 ? `2px ${STRIP_SIZE + 2}px 2px 2px` : i >= 14 && i <= 25 ? `2px 2px ${STRIP_SIZE + 2}px 2px` : i >= 27 && i <= 38 ? `2px 2px 2px ${STRIP_SIZE + 2}px` : `${STRIP_SIZE + 2}px 2px 2px 2px`) : "2px";
...
<div style={{ position: "absolute", top: 4, right: 4, display: "flex", gap: 2, flexWrap: "wrap", zIndex: 1 }}>
{occupants.map((p: any) => {
